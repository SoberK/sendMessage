...
const handle = async (userId) => {
  ...
  for (let index = 0 ; index < desList.length; index++) {
    const desId = desList[index].id;
    const desName = desList[index].name;
    const invoteRes = await fetch("https://m.taoke.com/mlearningapi.php", {
        "headers": {
          "accept": "application/json",
          "accept-language": "zh-CN,zh;q=0.9",
          "content-type": "application/x-www-form-urlencoded",
        ...
        "body": `p=%5B%7B%22m%22%3A%22Match%2FSupport%22%2C%22p%22%3A%7B%22match_id%22%3A451%2C%22target_id%22%3A%22${desId}%22%2C%22target_type%22%3A%22course%22%7D%7D%5D&global_company_id=69514`,
        "method": "POST",
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1"
    });

        const invoteJson = await invoteRes.json();
        try {
          const { msg } = invoteJson.d[0]
          if (msg) {
              console.log(`${userId}-${msg}`)
          } else {
              // console.log(invoteJson)
              console.log(`${userId} 投票${desName} 失败,可能已经投过`)
          }
        } catch (error) {
          console.log(userId, invoteJson)
          // await handle(userId)
        }
